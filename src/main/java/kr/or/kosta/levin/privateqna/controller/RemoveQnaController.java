...
/**
 * 1:1문의글 삭제를 위한 세부 컨트롤러
 * 
 * @author 류세은
 */

@Bean(type = BeanType.Controller)
@RequestMapping(value = "/privateqna/qna-remove")
public class RemoveQnaController implements Controller {
	...
	public Object handleRequest(HttpServletRequest request, HttpServletResponse response)
		...
		// 게시글번호 null이거나 빈값 들어오는 경우 체크
		if (articleId == null || articleId.trim().length() == 0) {
			throw new RequestBadRequestException();
		}

		try {
			removeQnaResult = qnaService.removeQna(Integer.parseInt(articleId));
			// 1:1문의글 삭제 성공 시
			if (removeQnaResult) {
				map.put("removeQnaResult", "true");
			} else {
				// 실패시
				map.put("removeQnaResult", "false");
			}
			return map;

		} catch (Exception e) {
			throw new ServletException("qnaService.removeQna() 예외 발생", e);
		}
	}
}
